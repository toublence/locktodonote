...
struct LockScreenTabView: View {
                ...
                LazyHStack(spacing: 10) {
                    ForEach(LockScreenTemplate.userFacingCases, id: \.self) { option in
                        ...
                        Button {
                            ...
                            VStack(alignment: .leading, spacing: 9) {
                                HStack {
                                    Image(systemName: option.symbolName)
                                        ...
                                        .foregroundStyle(isSelected ? palette.onPrimary : palette.accent)
                            ...
                            .padding(10)
                            .frame(
                                width: horizontalSizeClass == .regular ? 144 : 136,
                                height: 78,
                                alignment: .topLeading
                            )
                            .background(
                                isSelected ? palette.accentSoft : palette.surface,
    ...
    private var quickCapture: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(alignment: .bottom, spacing: 10) {
                TextField(
                    captureMode == .todo
                        ? appString(localized: "capture.todo.placeholder",
                            defaultValue: "Add todos — one per line"
                        )
                        : appString(localized: "capture.memo.placeholder", defaultValue: "Write a memo"),
                    text: $captureText,
                    axis: .vertical
                )
                .lineLimit(2...6)
                .frame(
                    minHeight: horizontalSizeClass == .regular ? 220 : 72,
                    alignment: .topLeading
                )
    ...
    private var todoSection: some View {
        DaySection(
            title: appString(localized: "home.todos", defaultValue: "Todos"),
            systemImage: "checklist",
            isEmpty: todoItems.isEmpty,
            emptyText: appString(localized: "home.noTodos", defaultValue: "No todos for today"),
            addLabel: appString(localized: "home.addTodo", defaultValue: "Add todo"),
            onAdd: { focusCapture(.todo) }
        ) {
            ForEach(todoItems, id: \.compositeId) { entry in
                TodoRowView(entry: entry) { isDone in
                    cardStore.setItemDone(cardId: entry.card.id, itemId: entry.item.id, isDone: isDone)
                    if isDone {
                        analytics.todoCompleted(
                            taskCount: todoItems.count,
                            remainingCount: todoItems.filter { !$0.item.isDone }.count - 1,
    ...
    private var memoSection: some View {
        DaySection(
            title: appString(localized: "home.memos", defaultValue: "Memos"),
            systemImage: "square.and.pencil",
            isEmpty: memoCards.isEmpty,
            emptyText: appString(localized: "home.noMemos", defaultValue: "No memos for today"),
            addLabel: appString(localized: "home.addMemo", defaultValue: "Add memo"),
            onAdd: { focusCapture(.memo) }
        ) {
    ...
    private func focusCapture(_ mode: QuickAddMode) {
        setCaptureMode(mode)
...
private struct LiveActivityCard: View {
    ...
    private var actionTitle: String {
        switch state {
        case .active: appString(localized: "liveActivity.refresh", defaultValue: "Refresh")
        case .possiblyExpired: appString(localized: "liveActivity.extend", defaultValue: "Extend")
        case .notStarted: appString(localized: "liveActivity.startShort", defaultValue: "Start")
        case .unsupported: appString(localized: "common.settings", defaultValue: "Settings")
        }
    }
...
private struct DaySection<Content: View>: View {
    let title: String
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Button(addLabel, action: onAdd)
                    ...
                    .buttonStyle(.plain)
                    .foregroundStyle(palette.accent)
                    .accessibilityIdentifier("section.add.\(systemImage)")
            }
...
extension LockScreenTemplate {
    ...
    var displayName: String {
        switch self {
        case .calendarItems: appString(localized: "template.calendarItems", defaultValue: "Calendar + Items")
        case .memoTodo: appString(localized: "template.memoTodo", defaultValue: "Memo + Todo")
        case .dateMemo, .dateTodo:
            appString(localized: "template.dateContent", defaultValue: "Date + Items")
        case .imageMemo, .imageTodo:
            appString(localized: "template.imageContent", defaultValue: "Image + Items")
